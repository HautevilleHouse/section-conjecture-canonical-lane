...
In-paper anchor: `paper/SECTION_CONJECTURE_PREPRINT.md` (`SC_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving existence and identification of rational sections of the étale fundamental exact sequence through an admissible section-packet closure architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`SC_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and admissible windows do not collapse.
This is the first-failure capture contribution to `M_SC`.
...
If near-failure extraction and no-Zeno spacing hold, then `SC_G3` prevents hidden drift before rigidity is applied.
